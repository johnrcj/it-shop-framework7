...
if (!function_exists('_make_thumbnail')) {
    /**
     * 썸네일파일 만들기
     *
    ...
    function _make_thumbnail($src_file_path, $story_id)
    {
        // 이미지파일경로를 파일명까지와 확장자로 분리한다.
...
//        // 썸네일 파일경로 얻기
//        $thumb_file_path = str_replace($org_filename, $story_id . "_thumb",  $src_file_path);
        $thumb_file_path = $story_id;

        switch ($image_info['mime']) {
            case "image/gif":
                $new_image = imagecreatefromgif($src_file_path);
                break;
            case "image/jpeg":
                $new_image = imagecreatefromjpeg($src_file_path);
                break;
            case "image/png":
                $new_image = imagecreatefrompng($src_file_path);
                break;
            case "image/bmp":
                $new_image = imagecreatefrombmp($src_file_path);
                break;
            default:
                $new_image = imagecreatefrompng($src_file_path);
        }
...
if (!function_exists('_hash_password')) {
    /**
     * 주어진 문짜열을 암호화한다.
     *
     *
    ...
    function _hash_password($plaintext)
    {
        return password_hash($plaintext, PASSWORD_DEFAULT);
    }
}

if (!function_exists('_verify_password')) {
    /**
     * 평문과 암호문짜열 비교결과를 리턴
     *
     *
    ...
    function _verify_password($plaintext, $hashedPassword)
    {
...
if (!function_exists('_make_phone_format')) {
    ...
    function _make_terms($kind) {
        ...
        switch ($kind) {
            case 1:
                $term_kind = "signup_nes";
                break;
            case 2:
                $term_kind = "signup_opt";
                break;
            case 3:
                $term_kind = "use_term";
                break;
            case 4:
                $term_kind = "refund_nes";
                break;
            case 5:
                $term_kind = "refund_opt1";
                break;
            case 6:
                $term_kind = "refund_opt2";
                break;
            case 7:
                $term_kind = "refund_opt3";
                break;
            case 8:
                $term_kind = "refund_opt4";
                break;
        }
    ...
    function _make_warning($kind) {
        ...
        switch ($kind) {
            case 1:
                $warning_kind = "refund_cash";
                break;
            case 2:
                $warning_kind = "check_cash";
                break;
            case 3:
                $warning_kind = "amount_cash";
                break;
            case 4:
                $warning_kind = "recognition";
                break;
        }
