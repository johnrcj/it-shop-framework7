<div class="row" style="margin-top: 20px;">
    <div class="col-md-12">
        <div class="width-100" style="text-align: right">
            <a href="<?=site_url("Terms/terms_detail?mode=1")?>" class="btn btn-primary"><i class="fa fa-plus"></i>&nbsp;<?=$this->lang->line("register")?></a>
        </div>

        <table id="tbl_terms" class="table table-bordered">
            <thead>
            <tr>
                ...
                <td class="center title_td" width="130px"><?= $this->lang->line("terms_kind") ?></td>
...
<script>
    ...
    $(document).ready(function () {
        $('#left_menu_terms_manage').addClass("active");
        $('#left_menu_terms_manage').children("a:eq(0)").children("span:eq(1)").addClass("selected");
        $('#page_title').html("<?=$this->lang->line('terms_manage')?>");
    })

    $(function () {
        oTable = $('#tbl_terms').DataTable({
            "stateSave": true,
            "processing": true,
            "serverSide": true,
            "autoWidth": false,
            ...
            "ajax": { // define ajax settings
                "url": "<?=site_url('Terms/ajax_table')?>", // ajax URL
                "type": "POST",
                "data": function (data) {
                },
            ...
            "createdRow": function (row, data, dataIndex) {
                ...
                $('td:eq(0)', row).bind("click", function () {
                    location.href = '<?=site_url("Terms/terms_detail")?>?uid=' + data['uid'] + '&mode=0';
                });
                $('td:eq(4)', row).html("<a href='<?=site_url("Terms/terms_detail")?>?uid=" + data['uid'] + "&mode=2'><i class='fa fa-edit' style='color: blue'></i></a>");
                $('td:eq(5)', row).html('<a onclick="onRemove(' + data['uid'] + ')"><i class="fa fa-trash" style="color: red"></i></a>');
            },
    ...
    function onRemove(uid) {
        showConfirmDlg("<?=$this->lang->line('do_you_want_to_delete')?>", function () {
            ajaxRequest('<?=site_url("Terms/delete_terms")?>', {
                uid: uid
            }, function (data) {
                if (data === "success") {
