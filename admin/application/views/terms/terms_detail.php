<div class="row" style="margin-top: 20px;">
    <div class="col-md-12">
        <form id="frm_detail" method="post" action="<?= site_url('Terms/update_terms') ?>"
            ...
            <table class="table table-bordered">
                ...
                <?php } ?>
                <tr>
                    <td class="title_td" style="width: 150px;"><?= $this->lang->line("terms_kind") ?>
                        <?php if ($mode != 0) { ?>
                        <span style="color: red">*</span>
                        <?php } ?>
                    <td class="padding_1">
                        ...
                        <input class="form-control" <?= $mode == 0 ? "readonly" : "" ?>
                               value="<?= $info != null ? $this->lang->line(_make_terms($info->kind)) : "" ?>">
                        <?php } else { ?>
                        <select id="ipt_kind" name="kind" class="form-control center">
                            <?php for ($i = 1; $i <= 8; $i++) { ?>
                            <option value="<?= $i ?>" <?= $info != null ? ($info->kind==$i ? "selected" : "") : "" ?>><?= $this->lang->line(_make_terms($i)) ?></option>
                            <?php } ?>
                        </select>
                        <?php } ?>
                    </td>
